// Parity check.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
...
std::string Parity_check(int input) 
{
    if (input % 2 == 0) 
    {
        return "Yes";
...
int main()
{
    ...
    for (int k = 1; k <= Tests;k++)
    {
        ...
        uniform_int_distribution<> dist_int3(0, 1000000);
        double from = -1.235;
        double to = 157.456;
        ...
        uniform_int_distribution<> dist_int4(-1000000, 10000000);
        double random_int;
        ...
        if (k < 10)
        {
            out1.open("C:\\Users\\menon\\source\\repos\\Parity check\\Parity check\\0" + std::to_string(k));
            out2.open("C:\\Users\\menon\\source\\repos\\Parity check\\Parity check\\0" + std::to_string(k) + ".a");
        }
        else
        {
            out1.open("C:\\Users\\menon\\source\\repos\\Parity check\\Parity check\\" + std::to_string(k));
            out2.open("C:\\Users\\menon\\source\\repos\\Parity check\\Parity check\\" + std::to_string(k) + ".a");
        }
        for (int i = 0; i < n;i++)
        {
            if (k < 3) {
        ...
        std::cout << Parity_check(input[0]) << "\n";
        out2 << Parity_check(input[0]) << '\n';
        out2.close();
    }
}
