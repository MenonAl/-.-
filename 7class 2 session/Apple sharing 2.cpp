// Apple sharing 2.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
//
#include <fstream>
#include <random>
...
int main()
{
    ...
    for (int k = 1; k <= Tests;k++)
    {
        ...
        if (k < 10)
        {
            out1.open("C:\\Users\\menon\\source\\repos\\Apple sharing 2\\Apple sharing 2\\0" + std::to_string(k));
            out2.open("C:\\Users\\menon\\source\\repos\\Apple sharing 2\\Apple sharing 2\\0" + std::to_string(k) + ".a");
        }
        else
        {
            out1.open("C:\\Users\\menon\\source\\repos\\Apple sharing 2\\Apple sharing 2\\" + std::to_string(k));
            out2.open("C:\\Users\\menon\\source\\repos\\Apple sharing 2\\Apple sharing 2\\" + std::to_string(k) + ".a");
        }
        for (int i = 0; i < n;i++)
        {
            if (k < 3) {
        ...
        if (input[0] < 0 || input[1] < 0) {
            ...
            if (input[1] = 0) 
            {
                ...
                ans = input[1] - (floor(input[1] / input[0]) * input[0]);
                out1.close();
                std::cout << input[0] << "\n" << input[1] << "\n" << ans << "\n";
                out2 << ans << '\n';
                out2.close();
